...
const search = async () =>{
  ...
  // const q = query(col, where("related_search", "==", text));
            ...
            <Box h="56" justifyContent="flex-end">
              ...
              >
                <VStack py="2">
                  ...
                  <Text color="coolGray.200">Results: organic, ads, inline image, inline videos, inline products, knowledge graph, local maps, inline twitter, related questions, related query</Text>
                </VStack>
              </HStack>
            </Box>
...
<></>
    ...
    {ItemFeeds.map((item, index) => {
        ...
        return ([
              item.ads.map((itemdata, index2) => {
                return([
                  itemdata.data.ads.top_text_ads.map((itemdata3, index3) => {
                    return(
                      <>
                      <text>Title : {itemdata3.title}</text>
                      <text>Ads : {itemdata3.url}</text>
                      </>
                    )
                  }),
                <div>
                <text>Metadata :{JSON.stringify(itemdata.data.query.related_search)} </text>
                <text>URL :{JSON.stringify(itemdata.data.query.related_search[0].url)} </text>
                <text>Result stat :{JSON.stringify(itemdata.data.result_stat)} </text>
              </div>
])
              })
            ])
