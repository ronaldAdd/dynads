...
//search token google ads
const col_ads = collection(db, "token");
const q_ads = query(col_ads, where("id", "==", '1'));
const querySnapshot_ads = await getDocs(q_ads);
console.log(querySnapshot_ads)
let array_ads=[];
querySnapshot_ads.forEach((doc) => {
  array_ads.push(doc.data())
});
...
const options = {
  ...
  headers: {
    ...
    'X-RapidAPI-Key': `${array_ads[0].goads}`
  }
};
...
<></>
    ...
    {ItemFeeds.map((item, index) => {
      ...
      try{
        title = JSON.stringify(item.ads.data.ads.top_text_ads[0].title);
        url = JSON.stringify(item.ads.data.ads.top_text_ads[0].url);
      ...
      return(
        ...
        <HStack alignItems="center" space="4">
          ...
          <Text
            ...
            _dark={{ color: 'coolGray.200' }}
            >
            Related Search : {JSON.stringify(item.ads.data.query.related_search)}
          </Text> 
          </VStack>                   
        </HStack>
      </Pressable>
      <Divider />
        ...
        // return ([
        //       <div>Text Ads : <text>{title}</text></div>,
        //       <div>URL : <text>{url}</text></div>,
        //       <div>Related search  : <text>{JSON.stringify(item.ads.data.query.related_search)}</text></div>,
        // ])
      })}                
              </VStack>
            </VStack>
